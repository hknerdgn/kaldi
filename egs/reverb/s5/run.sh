...
#reverb_tr=/export/ws15-ffs-data/corpora/reverb/REVERB/REVERB_WSJCAM0_tr_cut
...
if [ ! -e exp/mono0a/final.mdl ]; then
  ...
  steps/train_mono.sh --boost-silence 1.25 --cmd "$train_cmd" --nj $nj_train \
...
if [ ! -e exp/mono0a_ali/ali.1.gz ]; then
  ...
  steps/align_si.sh --boost-silence 1.25 --cmd "$train_cmd" --nj $nj_train \
...
if [ ! -e exp/tri1_ali/ali.1.gz ]; then
  ...
  # Re-align triphones.
  steps/align_si.sh --cmd "$train_cmd" --nj $nj_train \
...
if $do_tri2a; then
  ...
  for dataset in data/REVERB_dt/SimData_dt* data/REVERB_Real_dt/RealData_dt*; do
    steps/decode.sh --cmd "$decode_cmd" --nj $nj \
      exp/tri2a/graph_bg_5k $dataset exp/tri2a/decode_bg_5k_REVERB_dt_`basename $dataset` 
  done

  # decode REVERB dt using tri2a, mc
  for dataset in data/REVERB_dt/SimData_dt* data/REVERB_Real_dt/RealData_dt*; do
    steps/decode.sh --cmd "$decode_cmd" --nj $nj \
      exp/tri2a_mc/graph_bg_5k $dataset exp/tri2a_mc/decode_bg_5k_REVERB_dt_`basename $dataset` 
  done
  ...
  steps/get_fmllr_basis.sh --cmd "$train_cmd" \
    --per-utt true data/REVERB_tr_cut/SimData_tr_for_1ch_A data/lang exp/tri2a_mc 

  # Recognition using fMLLR adaptation (per-utterance processing).
  for dataset in data/REVERB_dt/SimData_dt* data/REVERB_Real_dt/RealData_dt*; do
    steps/decode_basis_fmllr.sh --cmd "$decode_cmd" --nj $nj \
      exp/tri2a_mc/graph_bg_5k $dataset exp/tri2a_mc/decode_basis_fmllr_bg_5k_REVERB_dt_`basename $dataset` 
  done
...
if [ ! -e exp/tri2c_mc/final.mdl ]; then
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" \
    ...
    $mct_nstates $mct_ngauss data/REVERB_tr_cut/SimData_tr_for_1ch_A data/lang exp/tri1_ali exp/tri2c_mc 
fi
...
for recog in tri2b tri2b_mc; do
  for lm in $lms; do
    ...
    if [ ! -e "$graph" ]; then
      ...
      utils/mkgraph.sh data/lang_test_$lm exp/$recog $graph 
    fi
  done
done
...
for base_recog in tri2b_mc; do
  ...
  if [ ! -e exp/$denlats_dir/.done.1 ]; then
    ...
    steps/make_denlats.sh --cmd "$train_cmd" --nj $nj_train \
...
do_adapt=true
lm="tg_5k" # lm="bg_5k" we can switch to bigram if you want to comapre the result with the HTK baseline
  ...
  for dataset in data/REVERB_dt/SimData_dt* \
    ...
    if $do_adapt; then
      if [ ! -e exp/$recog/fmllr.basis ]; then
        ...
        steps/get_fmllr_basis.sh --cmd "$train_cmd" \
	  --per-utt true data/$tr_dataset data/lang exp/$recog 
      fi
