...
for k=1:nchan*nsrc,
    for i=1:nchan,
        ...
        %    sproj(i,:)=sproj(i,:)+fftfilt(C(:,k,i).',s(k,:));
        %else
            sproj(i,:)=sproj(i,:)+filter(C(:,k,i).',1,s(k,:));
        %end
    end
end
...
% SDR
SDR=10*log10(sum(sum(s_filt.^2))/(eps+sum(sum((e_interf+e_artif).^2))));
% SIR
SIR=10*log10(sum(sum(s_filt.^2))/(eps+sum(sum(e_interf.^2))));
% SAR
SAR=10*log10(sum(sum((s_filt+e_interf).^2))/(eps+sum(sum(e_artif.^2))));
