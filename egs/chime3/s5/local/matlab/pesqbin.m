function [ scores ] = pesqbin( reference, degraded, fs, mode, binary )
...
%   Author: Kamil Wojcicki, UTD, November 2011
    ...
    % usage information
    usage = 'usage: [ pesq_mos ] = pesqbin( reference, degraded, fs, mode, binary );';

    % default settings and 'lite' input validation
    switch( nargin )
    case { 0, 1, 2 }, error( usage );
    case 3, mode='nb';  if isunix(), binary='pesq'; else, binary='pesq.exe'; end
    case 4, if isunix(), binary='pesq'; else, binary='pesq.exe'; end
    case 5,
    ...
    if isunix()
        ...
        scores = pesqbin_linux( reference, degraded, fs, mode, binary );
    else 
        % use Window-tailored routines
        scores = pesqbin_windows( reference, degraded, fs, mode, binary );
    end
...
function [ scores ] = pesqbin_linux( reference, degraded, fs, mode, binary )
    ...
    if ischar(reference)
        [ reference, fs, nbits ] = wavread( reference );
        file.reference = sprintf( '%s%s%d%s', tmpdir, filesep, pid, 'pesqbin_ref.wav' );
    else
        file.reference = sprintf( '%s%s%d%s', tmpdir, filesep, pid, 'pesqbin_ref.wav' );
    end % isstr(reference)
    ...
    if ischar(degraded)
        [ degraded, fs, nbits ] = wavread( degraded );
        file.degraded = sprintf( '%s%s%d%s', tmpdir, filesep, pid, 'pesqbin_deg.wav' );
    else
        file.degraded = sprintf( '%s%s%d%s', tmpdir, filesep, pid, 'pesqbin_deg.wav' );
    end % isstr(degraded)

    maxval = max ([abs(reference(:)'), abs(degraded(:)')]);
    wavwrite( 0.999*reference./maxval, fs, nbits, file.reference );
    wavwrite( 0.999*degraded./maxval, fs, nbits, file.degraded );
