...
na=`cat $data/feats.stftnmag.scp 2>/dev/null | wc -l`
...
if [ -f $data/feats.stftnmag.scp ] && [ $nf -ne $na ]; then
  echo "** split_data.sh: warning, #lines is (feats.scp,feats.stftnmag.scp) is ($nf,$na); you can "
  echo "**  use utils/fix_data_dir.sh $data to fix this."
fi
...
if [ ! -d $s1 ]; then 
  ...
  need_to_split=false
  for f in utt2spk spk2utt spk2warp feats.scp feats.stftnmag.scp text wav.scp cmvn.scp spk2gender \
...
for f in feats.scp feats.stftnmag.scp text vad.scp utt2lang $maybe_wav_scp; do
  if [ -f $data/$f ]; then
