...
if [ $# != 3 ]; then
   echo "usage: make_stft.sh [options] <data-dir> <log-dir> <path-to-stftdir>";
   echo "options: "
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_stft_${name}.JOB.log \
    ...
    compute-stft-feats --verbose=2 --config=$stft_config ark:- ark:- \| \
  ...
  $cmd JOB=1:$nj $logdir/make_stft_${name}.JOB.log \
    compute-stft-feats --verbose=2 --config=$stft_config scp,p:$logdir/wav.JOB.scp ark:- \| \
    copy-feats --compress=$compress ark:- \
     ark,scp:$stftdir/raw_stft_$name.JOB.ark,$stftdir/raw_stft_$name.JOB.scp \
     || exit 1;
