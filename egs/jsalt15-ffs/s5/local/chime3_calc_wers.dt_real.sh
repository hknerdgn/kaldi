...
echo ""
echo "Search scoring in outputs in $dir/decode_tgpr_5k_dt05_real_$enhan"
mkdir -p $dir/log

for a in $( find $dir/decode_tgpr_5k_dt05_real_$enhan/ -name 'wer_*' | awk -F/ '{print $NF}' | sort); do
    echo -n "$a "
...
for e_d in dt05; do # et05; do
  for task in simu real; do
	...
	cat $rdir/scoring/$lmw.tra \
	  | utils/int2sym.pl -f 2- $rdir/../../tri4a/graph_lm_tgpr_5k/words.txt \
	  | sed s:\<UNK\>::g \
