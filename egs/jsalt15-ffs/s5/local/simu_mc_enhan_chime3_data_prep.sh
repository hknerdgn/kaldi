...
if $eval_flag; then
list_set="tr05_simu_${processing} dt05_simu_${processing} et05_simu_${processing}"
else
list_set="tr05_simu_${processing} dt05_simu_${processing}"
fi

cd $dir

find $audio_dir -name *${channel}.wav | grep 'tr05_bus_simu\|tr05_caf_simu\|tr05_ped_simu\|tr05_str_simu' | sort -u > tr05_simu_${processing}.flist
find $audio_dir -name *${channel}.wav | grep 'dt05_bus_simu\|dt05_caf_simu\|dt05_ped_simu\|dt05_str_simu' | sort -u > dt05_simu_${processing}.flist
if $eval_flag; then
find $audio_dir -name *${channel}.wav | grep 'et05_bus_simu\|et05_caf_simu\|et05_ped_simu\|et05_str_simu' | sort -u > et05_simu_${processing}.flist
fi
...
cat tr05_simu_${processing}_wav.scp | awk -F'[_]' '{print $2}' | tr '[A-Z]' '[a-z]' \
    | $local/find_noisy_transcripts.pl dot_files.flist | cut -f 2- -d" " > tr05_simu_${processing}.txt
cat tr05_simu_${processing}_wav.scp | cut -f 1 -d" " > tr05_simu_${processing}.ids
