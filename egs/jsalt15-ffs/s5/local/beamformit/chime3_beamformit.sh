...
#in_enh=wpe6
#sdir=/export/ws15-ffs-data/mdelcroix/data/chime3/data_${in_enh}/data/audio/16kHz/isolated
#out_enh=${in_enh}_beamformed_1sec_scwin_ch1_3-6


odir=$resdir/data_${enh}/chime3/data/audio/16kHz/isolated

#./enhanced_wav/${sdir_name}_beamformed_1sec_scwin_ch1_3-6
wdir=data_${enh}/chime3/local
conf=local/beamformit/conf/beamformit.cfg
beamformitdir=/export/ws15-ffs-data/swatanabe/tools/beamformit/bin/BeamformIt-3.51
...
#for line in `find $sdir/ -name "*.CH1.wav" | egrep 'tr05|et05|dt05' | awk -F '/' '{print $(NF-1) "/" $NF}' | sed -e "s/.CH1.wav//" | sort`; do
# does not enhance train data!
for line in `find $sdir/ -name "*.CH1.wav" | grep $tset | awk -F '/' '{print $(NF-1) "/" $NF}' | sed -e "s/.CH1.wav//" | sort`; do
  channels="${line} "
  echo ${line} >> $wavfiles
  for ch in $bmf; do
...
$cmd JOB=1:$nj $wdir/log/beamform.JOB.log \
  `pwd`/local/beamformit/beamformit.sh $nj JOB $nbmics $wavfiles $sdir $odir $wdir $conf $beamformitdir
